...
{
    dsn_msg_update_request((void*)msg, timeout_milliseconds, hash);
}
...
(JNIEnv *env, jclass cla, jlong msg, jint timeout_milliseconds, jintArray phash)
{
    ...
    dsn_msg_query_request((void*)msg, &timeout, &hash);
    jint hash_jint = hash;
...
{
    void *ptr = NULL;
    size_t off = 0, buf_length, write_length;
    ...
    while (off < len)
    {
        ...
        memcpy_s(ptr, buf_length, buf + off, write_length);
...
(JNIEnv *env, jclass cla, jlong msg)
{
    return (jlong)dsn_msg_body_size((void*)msg);
}
